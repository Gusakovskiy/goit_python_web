...
async def test_login(db, auth_test):
    response = await auth_test.login()
    assert response.status == 200
...
async def test_login_validate_input(auth_test, db, username, password, message):
    response = await auth_test.login(username, password)
    assert response.status == 200
    data = await response.text()
    assert message in data


async def test_logout(auth_test):
    response = await auth_test.login()
    assert response.status == 200
    response = await auth_test.logout()
    assert response.status == 200
    user = auth_test.client.app.get('user')
    assert user is None
